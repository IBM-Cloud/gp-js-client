...
function Client(options) {
  ...
  // } else if ( !this._options.credentials.userName ) {
  //   throw new Error("gaas: missing 'credentials.api_key'  " + needAll);
  }
  
  if( this._options.credentials.serviceInstance ) {
    this._options.serviceInstance = this.options.credentials.serviceInstance;
  }
  
  if( !this._options.serviceInstance ) {
    // warn: no serviceInstance specified.
  }
...
Client.prototype.getBundleList = function getBundleList(opts, cb) {
  ...
  this.restCall("bundle.getBundleList", {serviceInstanceId: serviceInstance}, {}, function(err, restData, cbArg) {
      if(err) {
        deferred.reject(err);
      } else {
...
// /** @class Bundle */
// function Bundle(gaas, projectID, props) {
//   if ( props ) {
//     // copy properties to this
//     for(var k in props) {
//       this[k] = props[k];
