 ...
 * @param {Client~bundleListCallback} cb - callback
 */
...
**** TODO: ******
 ...
 // no userId
 client.users() ==>  array of User objects  
 
 client.createUser({  name, displayName, type … }  )  ==> User
 
 client.getUser({ userId: userId }) callback ==> user info
 
 User.delete() 
 User.update()
 User.getInfo() ==> refresh info
 
 
 gpClient.createUser({ displayname: "Yoshito" }, cb(err, User) {
      console.log(User.id);
      User.update()
      User.delete()
 }
 ...
 client.bundles() ==> array of Bundle objects
