...
const HTMLDocument = require('./htmldoc.js');
const MarkdownDocument = require('./mddoc.js');
...
class Client {
  ...
  restCall(fn, restArg) {
    ...
    return this.swaggerClient
      .then(client => {
        if(!client.spec) {
          return Promise.reject(Error('GP Swagger Error: could not load spec.'));
        }
        if(!client.apis || !Object.keys(client.apis).length) {
          return Promise.reject(Error('GP Swagger Error: client.apis did not load.'));
        }
        const group = client.apis[fn[0]];
        if(!group) return Promise.reject(Error('Not a REST group:' + fn[0]));
 ...
 */
  ...
  */
  docTr(opts) {
    ...
    return new DocumentTranslationRequest(this, opts);
  }
  ...
  docTrs(opts, cb) {
    opts = utils.fixArgs(arguments);
    if ((cb = arguments[1])) {
      return utils.depromise(this.docTrs(opts), cb);
    }
    const serviceInstance = this.getServiceInstance(opts);
    const summary = opts && opts.summary === true;
    const that = this;
    return this.restCall('translation_request.getDocumentTranslationRequests',
      {
        serviceInstanceId: serviceInstance,
        summary: summary
      }).then(data => {
      ...
      for (var n in trIds) {
        if (Object.prototype.hasOwnProperty.call(trIds, n)) {
          const trId = trIds[n];
          trs[trId] = that.docTr(data.translationRequests[trId]);
        }
      }
      return trs;
    });
  ...
  MarkdownDocument(id) {
    if(typeof id === 'string') {
      return new MarkdownDocument({id}, this);
    } else {
      // option bag
      return new MarkdownDocument(id, this);
    }
  ...
  HTMLDocument(id) {
    if(typeof id === 'string') {
      return new HTMLDocument({id}, this);
    } else {
      // option bag
      return new HTMLDocument(id, this);
    }
   ...
   * @returns Promise
   */
  async getMdDocumentList(opts) {
    const serviceInstanceId = this.getServiceInstance(opts);

    const {documentDataSet} = await this.restCall('document.getDocumentList', {
      serviceInstanceId,
      type: "MD"
    });
    // roll this up as we do for bundles etc
    return documentDataSet.reduce((p, v) => {
      p[v.documentId] = this.MarkdownDocument(v);
      return p;
    }, {});
  }
   ...
   * get a list of Markdown documents
   */
  async getHTMLDocumentList(opts) {
    const serviceInstanceId = this.getServiceInstance(opts);
    ...
    // roll this up as we do for bundles etc
    return documentDataSet.reduce((p, v) => {
      p[v.documentId] = this.HTMLDocument(v);
      return p;
    }, {});
  }
}
