 ...
 * @param {Object.<string,User>} users - user list
 * @see User
 */
...
//  * @param {listUsersCallback} cb - (err, map of resourceKey:ResourceEntry )
//  */
...
//   {
//       serviceInstanceId: this.getServiceInstance(opts)
//   }, function(err, data) {
//       if(err) return cb(err);
//       var users = {};
//       for(var uid in data.users) {
//           users[uid] = that.user(data.users[uid]);
//       }
 ...
 * 
 * @param {Object} opts - ignored
 * @param {listBundlesCallback} cb - given a map of Bundle objects
 */
 ...
 * @prop {String} type - ADMINISTRATOR|TRANSLATOR|READER
...
User.prototype.delete = function userDelete(cb) {
  this.gp.deleteUser({userId:this.id, 
  serviceInstanceId: this.gp.getServiceInstance(this)}, cb);
};
 ...
 * @param {Object} opts - options (currently ignored)
 * @param {basicCallback} cb - callback (err, ResourceEntry)
 */
