 ...
 * @param {string} updatedBy - information about how our service instance was updated
 * @param {date} updatedAt - when the instance was last updated
 * @param {string} region - the Bluemix region name
 * @param {string} cfServiceInstanceId - the CloudFoundry service instance ID
 * @param {string} serviceId - this is equivalent to the service instance ID
 * @param {string} orgId - this is the Bluemix organization ID
 * @param {string} spaceId - this is the Bluemix space ID
 * @param {string} planId - this is the Bluemix plan ID
 * @param {boolean} htServiceEnabled - true if the Human Translation service is enabled
 * @param {object} usage - usage information
 * @param {number} usage.size - the size of resource data used by the Globalization Pipeline instance in bytes
 * @param {boolean} disabled - true if this service has been set as disabled by Bluemix
 */
 ...
 */
Client.prototype.getServiceInstanceInfo = function getServiceInstanceInfo(opts, cb) {
  // make 'opts' optional
  if(!cb) {
      cb = opts;
      opts = {};
  ...
  var serviceInstance = this.getServiceInstance(opts);
  assert(cb !=null, 'cb must not be null');
  this.restCall("instance.getServiceInstanceInfo", {serviceInstanceId: serviceInstance}, function(err, data) {
    if(err) return cb(err);
    if(data.instance.updatedAt) {
      data.instance.updatedAt = utils.datify(data.instance.updatedAt);
    }
    return cb(null, data.instance);
  });
};
